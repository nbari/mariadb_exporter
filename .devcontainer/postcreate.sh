...
# One-time provisioning for the mariadb_exporter dev container (DevPod
# postCreateCommand): system deps, Rust components, and the mise-managed toolchain
# (just, mariadb client). The mariadb service runs as a compose sibling and is
# reached over the network at mariadb:3306 (see compose.yaml), so the app needs no
# container runtime of its own.
...
# 2. System dependencies (rustls build, no OpenSSL needed; the mariadb client comes
#    from the mise setup-mariadb-client task below). zsh is provided by the
#    common-utils devcontainer feature; make it the default shell for the slick
#    prompt + dotfiles.
sudo apt-get update
...
# 5. Install the MariaDB client (mariadb + mariadb-admin). Call the script directly
#    with an absolute path (not `mise run`) so it is independent of the task CWD.
#    setup-mariadb-client is required; setup-tig is an optional git browser.
bash "$REPO_ROOT/scripts/install-mariadb-client.sh"
...
echo "✓ postCreate complete: toolchain ready (just, rustfmt, clippy, mariadb client, slick)."
echo "  MariaDB is at \$MARIADB_HOST:\$MARIADB_PORT (mariadb:3306). Run: just test"
