# mariadb_exporter DevPod / Dev Containers workspace
...
(`compose.yaml`) defines the **app** dev container *and* the **mariadb** service it
...
```
   host: podman (or docker) + devpod + docker compose v2
     │  devpod up .
     ▼
 ┌──────────── compose project "mariadb_exporter" ───────────┐
 │  app  (service "app", user vscode)                        │
 │   └─ rust + mise (just, mariadb client, cargo-edit)       │
 │                                                           │
 │  mariadb:3306  (root password: root, db: mysql)           │
 └───────────────────────────────────────────────────────────┘
```

Inside the project network the app reaches the database **by name** at
`mariadb:3306` (container-to-container). There is **no host port-forward** in the
path, which means:

- no rootless-podman/`pasta` bulk-`COPY` stalls,
...
| File | Purpose |
| --- | --- |
| `compose.yaml` | The stack: `app` + `mariadb` (+ optional Prometheus/Grafana). |
...
| `init-db.sql` | First-boot SQL: least-privilege `exporter` user + small seed database. |
| `postcreate.sh` | One-time provisioning: system deps, rustup components, `mise install`, dotfiles. |
| `post-start.sh` | Every start: wait for mariadb, seed the test DB. |
| `../scripts/dev-ssh` | Host helper for entering or running commands in `/workspaces/mariadb_exporter`. |
...
The toolchain is declared in [`../mise.toml`](../mise.toml) (just, cargo-edit, the
[slick](https://github.com/nbari/slick) prompt, tree-sitter, and the mariadb client
task). Neovim is installed via a devcontainer feature, and `postcreate.sh` applies
your dotfiles with [chezmoi](https://chezmoi.io) (repo from `DEVPOD_DOTFILES`,
...
scripts/dev-ssh              # shell in as vscode, in /workspaces/mariadb_exporter
# inside the container:
just test                    # runs against the mariadb service (no host DB needed)
```

`scripts/dev-up` runs `devpod up . --ide none --id mariadb-exporter --ssh-config
...
`devpod ssh mariadb-exporter --workdir /workspaces/mariadb_exporter` to enter the
workspace.
...
devpod up . --devcontainer-path .devcontainer/devcontainer.portable.json \
  --ide none --id mariadb-exporter
devpod ssh mariadb-exporter --workdir /workspaces/mariadb_exporter
```

The portable config still starts `app` + `mariadb`; it only omits
`compose.podman.yaml`.
...
`devpod up` (which starts only `app` + `mariadb`) stays lean — they are **not**
started by default.

```bash
# inside the container: run the exporter so app:9306 has data
just watch

# on the host: bring up Prometheus + Grafana (joins the devcontainer network)
just metrics-dev          # -> Grafana http://localhost:3001, Prometheus :9091
just metrics-dev-stop     # stop them when done
```

Why this is the agnostic approach:

- Prometheus scrapes the exporter by **service name** (`app:9306`) over the shared
...
created and attaches Prometheus/Grafana to that exact network.

## How the database connection is wired

`compose.yaml` sets these in the `app` service, and the tooling honors them
(defaulting to `127.0.0.1` outside the container, so the host workflow is unchanged):
...
| `MARIADB_USER` | `root` |
| `MARIADB_PASS` | `root` |
| `MARIADB_EXPORTER_DSN` | `mysql://root:root@mariadb:3306/mysql` |

> Root over TCP requires the password (`root@'%'`); a passwordless DSN is rejected
> by the server, so the DSN embeds it. `just test` honors a pre-set
> `MARIADB_EXPORTER_DSN`, falling back to the host default when unset.

## Notes

- **Multi-arch:** the base images (`devcontainers/rust`, `mariadb`) are multi-arch,
