...
//!
//! ```promql
//! (mariadb_system_memory_total_bytes - mariadb_system_memory_available_bytes)
...
impl MemoryCollector {
    ...
    pub fn new() -> Self {
        ...
        .expect("mariadb_system_memory_total_bytes");
...
impl Collector for MemoryCollector {
    ...
    /// Always `Fresh`: `sysinfo` reads kernel memory counters that are always
    /// present on every supported platform, so there is no "source unavailable"
    /// path to settle.
...
mod tests {
    ...
    #[test]
    fn reset_metrics_keeps_scalar_gauges_registered() {
        let collector = MemoryCollector::new();
        let registry = Registry::new();
        ...
        // Scalars cannot be removed once registered; the contract for this
        // collector is that it never skips, so the family must stay present.
        assert!(
            registry
                .gather()
                .iter()
                .any(|f| f.name() == "mariadb_system_memory_total_bytes"),
            "memory family must remain after a no-op reset"
        );
    }
}
