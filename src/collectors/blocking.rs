...
//!
//! A few collectors sample the operating system rather than `MariaDB`, and every one of
//! those APIs is **blocking**: `std::fs` on `/proc` (Linux), `sysctlbyname` (FreeBSD), and
//! `sysinfo` refreshes. None of it is async, and none of it yields.
//!
...
//! That is what made `nbari/pg_exporter#35` so hard to attribute in the sibling exporter: a
//! `/proc` problem presented as a database connectivity problem.
//!
//! [`offload_coalesced`] moves the work to Tokio's blocking pool, where a long sample
//! degrades into a merely slow collector instead of a runtime-wide outage. It also gives
//! the scrape task real await points, so the scrape timeout can fire and stop waiting on
//! it.
//!
...
//! [`offload_coalesced`]. It caps each reader at one submitted sample, so a read that
//! outlives its scrape (or never returns) cannot pile started or queued tasks onto the
//! pool.
...
/// interval — a PSS walk slower than the timeout, or a `/proc` read on a host where an
/// endpoint-security agent taxes every file I/O — those tasks pile up on the blocking pool
/// without bound (each started one also holds a pool thread while it waits). With the slot,
/// at most one sample per collector is ever in flight; a hung read leaks exactly one
/// blocking thread until it returns, and concurrent scrapes keep the previous gauges
/// instead of queueing duplicate work.
///
/// The guard is acquired with `try_lock` *before* spawning and moved into the closure, so a
/// hung closure holds it for as long as it is stuck and the skip path stays cheap on the
/// runtime worker.
///
/// # Errors
///
/// Same as [`offload`] when the work runs.
pub async fn offload_coalesced<F, T>(
...
mod tests {
    ...
    /// The core of `nbari/pg_exporter#35`: a long sample must not stop the runtime from
    /// making progress. Pinned to a single-threaded runtime so there is exactly one worker;
    ...
    async fn a_slow_sample_does_not_starve_the_runtime() -> Result<()> {
        ...
        assert!(
            ...
            "the runtime made no progress during a 300ms sample ({before} -> {after}): the \
             ...
             surfaces as bogus 'pool timed out' errors (nbari/pg_exporter#35)"
        );
        Ok(())
    }
