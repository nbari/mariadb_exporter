...
#[instrument(skip(pool), err, fields(db.system="mysql", db.operation="ping", otel.kind="client"))]
async fn check_database_health(pool: &MySqlPool) -> Result<(), StatusCode> {
    // Acquire connection
...
pub async fn health(method: Method, pool: Extension<MySqlPool>) -> impl IntoResponse {
