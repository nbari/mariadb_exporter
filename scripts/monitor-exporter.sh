#!/usr/bin/env bash
#
# monitor-exporter.sh - Monitor CPU and memory usage of mariadb_exporter
#
# Usage:
...
#   -p, --pid PID            Monitor specific PID (default: find mariadb_exporter)
#   -j, --json               Output in JSON format
#   -h, --help               Show this help message
#
# Examples:
#   # Monitor mariadb_exporter for 60 seconds, 5-second intervals
#   ./scripts/monitor-exporter.sh -d 60
#
#   # Monitor specific PID, output to file
#   ./scripts/monitor-exporter.sh -p 12345 -o /tmp/mariadb_exporter_metrics.log
#
...
# Function to find mariadb_exporter PID
find_mariadb_exporter_pid() {
    local pids
    pids=$(pgrep -f 'mariadb_exporter|target.*mariadb_exporter' 2>/dev/null || true)

    if [[ -z "$pids" ]]; then
        echo "Error: mariadb_exporter process not found" >&2
        echo "Hint: Start mariadb_exporter first or specify PID with -p" >&2
        exit 1
    fi
...
if [[ -z "$PID" ]]; then
    PID=$(find_mariadb_exporter_pid)
    echo -e "${GREEN}Found mariadb_exporter with PID: ${PID}${NC}" >&2
fi
